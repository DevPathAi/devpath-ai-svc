...
const OCI_SOURCE = 'https://github.com/DevPathAi/devpath-ai-svc';
const REPOSITORY = 'ghcr.io/devpathai/devpath-ai-svc';
...
test('404 requires the exact manifest endpoint and MANIFEST_UNKNOWN envelope', async (t) => {
  const mutations = [
    ...
    },
    {
      ...
      response: jsonResponse({
        errors: [{
          ...
          detail: { repository: 'devpath-ai-svc' },
        }],
      }, { status: 404 }),
      error: /MANIFEST_UNKNOWN/,
    },
    {
      ...
      url: 'https://ghcr.io/v2/devpathai/devpath-ai-svc/manifests/wrong',
      error: /response URL/,
    },
  ];
...
test('an OCI index must contain exactly one runnable linux/amd64 image', async () => {
  const config = JSON.stringify({
    architecture: 'amd64',
    os: 'linux',
    rootfs: { type: 'layers', diff_ids: [`sha256:${'c'.repeat(64)}`] },
  ...
  const child = JSON.stringify({
    schemaVersion: 2,
    mediaType: 'application/vnd.oci.image.manifest.v1+json',
    config: { digest: configDigest, size: Buffer.byteLength(config) },
    layers: [],
  });
