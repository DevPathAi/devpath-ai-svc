...
export async function inspectImmutableTag(options) {
  ...
  if (expectedOciSource !== 'https://github.com/DevPathAi/devpath-ai-svc') {
...
async function main() {
  ...
  if (arguments_.evidencePath) {
    if (result.state !== 'present') fail('cannot write evidence for an absent tag');
    mkdirSync(dirname(arguments_.evidencePath), { recursive: true });
    writeFileSync(arguments_.evidencePath, `${JSON.stringify(result, null, 2)}\n`, {
      encoding: 'utf8',
      mode: 0o600,
    });
  }
